...
f_names_file = r'Med_projekts\testa_datu_generacija\dati_testam_meitenses.txt'
m_names_file = r'Med_projekts\testa_datu_generacija\dati_testam_zeni.txt'
f_lastnames_file = r'Med_projekts\testa_datu_generacija\dati_testam_u_meitenes.txt'
m_lastnames_file = r'Med_projekts\testa_datu_generacija\dati_testam_u_zeni.txt'
slimibas_file = r'Med_projekts\testa_datu_generacija\dati_hroni_slim.txt'
vakcinas_file = r'Med_projekts\testa_datu_generacija\dati_vakcinas.txt'

datu_baze = r'Med_projekts\datubaze.db'
...
with db.connect(datu_baze) as con:
    for i in range(1,13):
        for j in range(0,random.randrange(2,5)):
            for k in range(0,27):
                cur = con.execute("""INSERT INTO skolenu_saraksts (klase, vards_uzvards ,pk ,dz_dati, tel_nr, med_karte, hroniskas_sl, trukst_vakc) VALUES(?,?,?,?,?,?,?,?)
                """,(str(i)+'.'+burti[j],generate_name(),generate_pk(i),generate_datums(i),generate_tel(),generate_medkarte(),generate_slimibas(),generate_vakc()))